...
class DAE:
    ...
    def init_parameters(self):
        ...
        with tf.name_scope('weights'):
            self.W_1 = tf.get_variable(name='weight_1',
                                       shape=(self.FLAGS.num_v, self.FLAGS.num_h),
                                       initializer=self.weight_initializer)
            self.W_2 = tf.get_variable(name='weight_2',
                                       shape=(self.FLAGS.num_h, self.FLAGS.num_h),
                                       initializer=self.weight_initializer)
            self.W_3 = tf.get_variable(name='weight_3',
                                       shape=(self.FLAGS.num_h, self.FLAGS.num_h),
                                       initializer=self.weight_initializer)
            self.W_4 = tf.get_variable(name='weight_4',
                                       shape=(self.FLAGS.num_h, self.FLAGS.num_v),
                                       initializer=self.weight_initializer)
    ...
    def _inference(self, x, is_training=True):
        ...
        with tf.name_scope('inference'):
            ...
            # a1 = tf.nn.dropout(a1, 0.3)
            a2 = tf.nn.relu(tf.nn.bias_add(tf.matmul(a1, self.W_2), self.b2))
            a2 = tf.nn.dropout(a2, 0.3)
            # a2 = tf.layers.batch_normalization(a2, training=is_training)
            a3 = tf.nn.relu(tf.nn.bias_add(tf.matmul(a2, self.W_3), self.b3))
            # a3 = tf.nn.dropout(a3, 0.3)
    ...
    def _validation_loss(self, x_train, x_test):
        ...
        outputs = self._inference(x_train, False)  # use training sample to make prediction
